...
-- =======================================================
-- Local Postgres compatible schema for MIS
-- Replaces Supabase auth references (auth.users, auth.uid()) with public.users
-- Adds helper current_user_id() that reads session setting 'app.current_user_id'
-- =======================================================

-- EXTENSIONS
create extension if not exists "uuid-ossp";

-- Local users table (replace Supabase auth.users)
...
-- Helper function to keep updated_at current for tables
...
-- PROFILES (now referencing public.users)
...
-- SESSION-BASED CURRENT USER HELPERS
-- NOTE: This reads a session setting 'app.current_user_id'. Your application should set this
-- per-connection (or per-transaction) with: SET LOCAL app.current_user_id = 'uuid-value';
-- If not set, current_user_id() returns NULL.
...
select coalesce((select role from public.profiles where id = public.current_user_id()), '') = target;
$$;

create or replace function public.has_any_role(targets text[]) returns boolean language sql stable as $$
select (select role from public.profiles where id = public.current_user_id()) = any(targets);
$$;
...
select 
  ...
  coalesce(
    sum(
      case when t.direction = 'ISSUE' then t.qty else -t.qty end
    ) over (partition by t.part_id order by t.created_at),
  0) as balance,
  t.asset_id,
...
-- Enable RLS for tables (Postgres supports RLS). You must create policies appropriate to your app.
...
---------------------------------------------------------
-- Tracks which spare parts were used in each breakdown
create table if not exists public.breakdown_spares_used (
  id bigserial primary key,
  breakdown_id bigint references public.breakdown_logs(id) on delete cascade,
  part_code text references public.spare_parts_inventory(part_code) on delete set null,
  used_qty numeric(14,3) not null check (used_qty > 0),
  created_at timestamptz default now()
);
create or replace function update_spare_stock_after_breakdown()
returns trigger as $$
begin
  update public.spare_parts_inventory
  set stock_on_hand = stock_on_hand - NEW.used_qty,
      updated_at = now()
...
$$ language plpgsql;
create trigger trg_update_spare_stock
after insert on public.breakdown_spares_used
for each row
execute function update_spare_stock_after_breakdown();
